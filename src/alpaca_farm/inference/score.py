...
# Additions for reward model test (IGNORE).

# @torch.inference_mode()
# def rerank_single_sequence_with_huggingface(
#     sequence: str,
#     model_name_or_path: str,
#     per_device_batch_size=20,
#     cache_dir=constants.DEFAULT_CACHE_DIR,
...
#     flash_attn=False,
# ) -> float:
#     """Calculate single reward for a sequence with a reward model.
#     """
#     model, tokenizer = load_model_and_tokenizer_for_inference(
#         model_name_or_path=model_name_or_path,
#         model_cls=reward_model.RewardModel,
...
#     )
#     source = common.prepare_inputs(source, device=device)
#     reward = model(input_ids=source.input_ids, attention_mask=source.attention_mask).rewards
#     return reward.item()