...
def decode_prompts_and_outputs_with_huggingface(
    ...
    **decoding_kwargs,
    ...
    Args:
        prompts: A sequence of string prompts.
        outputs: A sequence of string outputs.
        decoding_args: Decoding arguments.
        model_name_or_path: The name or path of the huggingface model. If it is a path, the directory location should also store
            the tokenizer.
        per_device_batch_size: The batch size per device for decoding.
        max_token_size: The maximum token size for cutoff.
    ...
    return decode_prompts_and_outputs_with_huggingface_given_model(
        model=model,
        tokenizer=tokenizer,
        prompts=prompts,
        outputs=outputs,
...
@torch.inference_mode()
def decode_prompts_and_outputs_with_huggingface_given_model(
    model: transformers.PreTrainedModel,
    tokenizer: transformers.PreTrainedTokenizer,
    prompts: Sequence[str],
    outputs: Sequence[str],
